  ...
  static Future<Homography?> transformImage(
    ...
    final referenceFrame = await cv.imreadAsync(
        (await TimelapseFrame.fromExisting(
            ...
            .path);
    final referenceGray =
        await cv.cvtColorAsync(referenceFrame, cv.COLOR_BGR2GRAY);
    ...
    final scrPts = cv.Mat.from2DList(
            goodMatches.map((e) => [kpRef[e.queryIdx].x, kpRef[e.queryIdx].y]),
            cv.MatType.CV_32FC1)
        .reshapeTo(1, [goodMatches.length, 1, 2]);
    final dstPts = cv.Mat.from2DList(
            goodMatches.map((e) => [kpImg[e.queryIdx].x, kpImg[e.queryIdx].y]),
            cv.MatType.CV_32FC1)
        .reshapeTo(1, [goodMatches.length, 1, 2]);

    final (homography, _) = await cv.findHomographyAsync(dstPts, scrPts);
